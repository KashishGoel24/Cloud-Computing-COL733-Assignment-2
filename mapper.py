...
class MapperState:
  ...
  def recover(self, recovery_id: int, checkpoint_id: int):
    ...
    if (checkpoint_id == -1):
      self.last_cp_id = 0
      self.last_stream_id = b"0"                              # there is no cp created so just set the initial stream id to 0
    else:
      self.last_cp_id = checkpoint_id
      filename = f"checkpoints/{self.id}_{checkpoint_id}.txt"
      try:
        if os.path.exists(filename):
          with open(filename, 'r') as file:
            ...
            stream_id = checkpoint_data.split('-')[0]
            self.last_stream_id = stream_id.encode()
      except Exception as e:
        logging.error(f"Error while recovering from checkpoint {checkpoint_id}: {e}")
    # TODO
  ...
  def recoverConnectionWithReducers(self):
    for i in range (len(self.reducer_sockets)):
      # check if the socket is closed
      sock = self.reducer_sockets[i]
      sock_closed = False
      # here put the code to check if the socket connection is closed and if yes then set sock_closed to Tue
      try:
      ...
      # if the socket is indeed closed
      if (sock_closed):
        logging.info(f"Connection was broken with Reducer_{i}, re-establishing the connection")
        conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        while True:
          try:
            conn.connect(("localhost", self.reducer_ports[i]))
            self.reducer_sockets[i] = conn
            logging.info(f"Succesfully reconnected with Reducer_{i}")
            break
          except ConnectionRefusedError:
            # Keep trying, eventually reducer will be up.
            logging.warning("Couldn't connect to reducer, waiting for it go up.")
...
class Checkpoint(Cmd):
  ...
  def handle(self, state: MapperState):
    if ((self.recovery_id == state.last_recovery_id) and (self.checkpoint_id==0 or (self.checkpoint_id>state.last_cp_id))):
      logging.info(f"{state.id} creating the checkpoint of id no {self.checkpoint_id}") 
      state.checkpoint(self.checkpoint_id)          # update to the last_cp_id and the last_stream_id is happening in this itself
      for i in range (len(state.reducer_sockets)):
        try:
          r_sock = state.reducer_sockets[i]
          msg_bytes = Message(msg_type=MT.FWD_CHECKPOINT, source=state.id, checkpoint_id=self.checkpoint_id, recovery_id=self.recovery_id, last_recovery_id=state.last_recovery_id).serialize()
          r_sock.sendall(msg_bytes)
        except Exception as e:
          logging.error(e)
          time.sleep(0.1)
      if (self.checkpoint_id == 0):
        last_cp_ack_msg = Message(msg_type=MT.LAST_CHECKPOINT_ACK, source=state.id, checkpoint_id=0, recovery_id=self.recovery_id)
        state.to_coordinator(last_cp_ack_msg)
      else:
        cp_ack_msg = Message(msg_type=MT.CHECKPOINT_ACK, source=state.id, checkpoint_id=self.checkpoint_id, recovery_id=self.recovery_id)
        state.to_coordinator(cp_ack_msg)
...
class Recover(Cmd):
  ...
  def handle(self, state: MapperState):
    logging.info(f"{state.id} recovering from {self.checkpoint_id} with recovery number {self.recovery_id}")
    state.recover(self.recovery_id, self.checkpoint_id)
    # sending the recovery acknowledgement to coordinator
    state.is_wc_done = False
    # checking if a reducer connection is down and if yes then recivering it
    state.recoverConnectionWithReducers()
    recovery_ack_msg = Message(msg_type=MT.RECOVERY_ACK, source=state.id, recovery_id=self.recovery_id)
    state.to_coordinator(recovery_ack_msg)
