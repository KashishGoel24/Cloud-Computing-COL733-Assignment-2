...
class ReducerState:
  ...
  def recover(self, recovery_id: int, checkpoint_id: int):
    ...
    if (checkpoint_id == -1):
      # there is no cp created so just set the wc dictionary to empty dict
...
class CPMarker(Cmd):
  def __init__(self, checkpoint_id: int, recovery_id: int, last_recovery_id: int):
    self.checkpoint_id = checkpoint_id
    self.recovery_id = recovery_id
    self.last_recovery_id_of_mapper = last_recovery_id
  ...
  def handle(self, state: ReducerState):
    if ((self.recovery_id == state.last_recovery_id) and ((self.checkpoint_id==0) or (self.checkpoint_id>state.last_cp_id))):
      ...
      if (self.checkpoint_id == 0):
        state.to_coordinator(Message(msg_type=MT.LAST_CHECKPOINT_ACK, source=state.id, checkpoint_id=0, recovery_id=self.recovery_id))
        logging.info(f"{state.id} completed last checkpoint and sent last_checkpoint_ack")
      else:
        state.to_coordinator(Message(msg_type=MT.CHECKPOINT_ACK, source=state.id, checkpoint_id=self.checkpoint_id, recovery_id=self.recovery_id))
        logging.info(f"{state.id} completed checkpoint {self.checkpoint_id} and sent checkpoint_ack")
    ...
    try:
      while True:
        ...
        if message.msg_type == MT.FWD_CHECKPOINT:  # received checkpoint marker
          try:
            logging.info(f"{self.id} received cp marker from {message.source}")
            if (message.source == "Mapper_0"):
              self.cp_marker[0] = message.kwargs["checkpoint_id"]
            else:
              self.cp_marker[1] = message.kwargs["checkpoint_id"]
            barrier.wait()            ####### check how this is working not very sure of the implementation
            if (self.cp_marker[0] == self.cp_marker[1]):
              checkpoint_num = message.kwargs["checkpoint_id"]
              logging.info(f"{self.id} putting the cp marker for reducer in the queue with cp id {checkpoint_num}")
              cmd_q.put(CPMarker(checkpoint_id=message.kwargs["checkpoint_id"], recovery_id=message.kwargs["recovery_id"], last_recovery_id=message.kwargs["last_recovery_id"]))
          except threading.BrokenBarrierError as e:
            logging.error(f"Barrier broken: {e}")
          # TODO
          # pass
        else:
