...
class CoordinatorState():
  def __init__(self) -> None:
    ...
    self.lastAckedWorkers: int = 0
    self.reducersRecovered: int = 0
    self.mappersRecovered: int = 0
    self.ackedWorkersForCurrentCP: int = 0
    self.toRecoverCheckpointNumber: int = -1
...
class CkptAckRecvMsg(RcvMsg):
  def __init__(self, checkpoint_id: str, recovery_id: int):
    self.checkpoint_id: Final[int] = int(checkpoint_id)
    self.recovery_id = recovery_id

  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    if self.recovery_id == state.next_recovery_id:
      logging.info(f"Coordinator has received the checkpoint ack from {source}")
      if (self.checkpoint_id == state.next_cp_id):
      ...
      for _, ws in state.workers.items():
        if ws.last_cp_id != self.checkpoint_id:
          allAcked = False
      if (allAcked):
        logging.info(f"coordinator got the ack from all of the workers for {self.checkpoint_id}") 
        state.next_cp_id += 1
        # if (state.phase != PHASE.LAST_CP):
        return PHASE.CP
    return None
    # pass
...
class LastCkptAckRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    if self.recovery_id == state.next_recovery_id:
      logging.info(f"Coordinator received the last checkpoint ack from {source}")
      if (not state.workers[source].last_checkpoint_done):
        state.workers[source].last_checkpoint_done = True
        state.lastAckedWorkers += 1
      allAcked = True
      for _, ws in state.workers.items():
        if not ws.last_checkpoint_done:
          allAcked = False
      if (allAcked):
        logging.info(f"Coordinator is incrementing the next_cp_id") 
...
class RecoveryAckRecvMsg(RcvMsg):
  ...
  def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
    logging.info(f"Coordinator received the Recovery ack from {source}")
    if (state.workers[source].is_mapper):
      state.workers[source].recovery_id = self.recovery_id
      allMappersRecovered = True
      for _, ws in state.workers.items():
        if ((ws.is_mapper) and (ws.recovery_id != self.recovery_id)):
          allMappersRecovered = False
      if (allMappersRecovered):
        return PHASE.CP
      return None
    else:
      state.workers[source].recovery_id = self.recovery_id
      allReducersRecovered = True
      for _, ws in state.workers.items():
        if ((not ws.is_mapper) and (ws.recovery_id != self.recovery_id)):
          allReducersRecovered = False
      if (allReducersRecovered):
        logging.info(f"Coordinator changing the phase to recover mapper phase")
...
class RecvThread(threading.Thread):
  ...
  def monitor_health(self, signum, frame):
    ...
    if recover:
      time.sleep(0.2)
      # here i need to change the phase but i feel that is not sufficient
      # we will have to append the phase in the phase_queue as well just like it is being done in the run function
      # have to also restart the crashed worker
      logging.info(f"changing the phase from {self.state.phase} to RECOVER_REDUCER phase")
      new_phase = PHASE.RECOVER_REDUCER
      self.state.phase = new_phase                # check if i should do this or just put it in the phase queue
      self.phase_queue.put(new_phase)
      self.state.next_recovery_id += 1
      self.state.toRecoverCheckpointNumber = self.state.last_completed_checkpoint_id()
      self.state.reducersRecovered = 0
      self.state.mappersRecovered = 0
      self.lastAckedWorkers = 0
      self.ackedWorkersForCurrentCP = 0
...
class SendThread(threading.Thread):
  ...
  def cp_phase(self):
    ...
    for _, ws in self.state.workers.items():
      if (ws.is_mapper):
        logging.info(f"{self.id} sending cp markers to the mappers")
  ...
  def recover_phase(self, is_mapper: bool) -> None:
    if is_mapper:
      ...
      for _, ws in self.state.workers.items():
        if (ws.is_mapper):
          ws.reset()
          ws.last_cp_id = self.state.toRecoverCheckpointNumber
          logging.info(f"{self.id} sending recovery messages to all the mappers") 
          RecoveryMsg(self.state.toRecoverCheckpointNumber, self.state.next_recovery_id).send(self.state.sock, ws.addr)
    else:
      ...
      for _, ws in self.state.workers.items():
        ...
        if (not ws.is_mapper):
          ws.last_cp_id = self.state.toRecoverCheckpointNumber
          logging.info(f"{self.id} sending recovery messages to all the reducers") 
          RecoveryMsg(self.state.toRecoverCheckpointNumber, self.state.next_recovery_id).send(self.state.sock, ws.addr)
  ...
  def last_cp_phase(self):
    ...
    for worker_id, worker in self.state.workers.items():
      if worker.is_mapper and not worker.is_done:
        rightState = False
        break
    if not rightState:
      pass
    # here you just send the last checkpoint marker to the mappers
    # then once the mappers ack that they have created the last_cp the update function handles itself
    logging.info(f"{self.id} sending last checkpoint create command to mappers")
    for _, ws in self.state.workers.items():
      if (ws.is_mapper):
  ...
  def exit_phase(self, start_time):
    ...
    rightState = True
    for worker_id, worker in self.state.workers.items():
      if not worker.last_checkpoint_done:
        rightState = False
        break
    if not rightState:
      pass
